    ...
    <React.Fragment>
      ...
      <List disablePadding>
        ...
        {Object.keys(tickets).map(key => (
          <ListItem className={classes.listItem} key={key}>
            ...
            <Typography variant="body2">${tickets[key].price}*{tickets[key].amount}</Typography>
          </ListItem>
        ))}

        <ListItem className={classes.listItem}>
          <ListItemText primary="Total" />
          <Typography variant="subtitle1" className={classes.total}>${total}</Typography>
        </ListItem>
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} sm={6}>
          <Typography variant="h6" gutterBottom className={classes.title}>Contact</Typography>
        ...
        <Grid item container direction="column" xs={12} sm={6}>
          ...
          <Grid container>
            {paymentId === 0 ? (
              ...
              <Grid item xs={12}>
                {payment &&
                  <Typography gutterBottom>[{payment.nameOnCard}] Ending in {payment.cardNumber}</Typography>}
              </Grid>
            }
          </Grid>
